...
impl<const MAX_DEPTH: usize, const MAX_BUFFER_SIZE: usize> Tree<MAX_DEPTH, MAX_BUFFER_SIZE> {
    ...
    pub fn merkle_roll_account_size(&self) -> u64 {
        ...
        let changelog_size = (MAX_DEPTH * 32 + 32 + 4 + 4) * MAX_BUFFER_SIZE;
        ...
        // This conversion is not expected to fail.
        u64::try_from(merkle_roll_size + header_size + canopy_size).unwrap()
    }
    ...
    pub async fn decode_root(&self) -> Result<[u8; 32]> {
        ...
        let roll = try_from_bytes::<MerkleRoll<MAX_DEPTH, MAX_BUFFER_SIZE>>(roll_bytes)
            .map_err(Error::Pod)?;
